        ...
        if (ret is True) and (frameCounter in framesToCompute) and frameCounter >= warm_start:
            ...
            if world2ref_transform is not None:
                ...
                plot_images([cv2.cvtColor(world_frame, cv2.COLOR_GRAY2BGR),
                            ...
                            ['World frame {}'.format(
                                frameCounter), 'Reference frame'],
                            pad=0.5)
                # plot_color_line_matches(line_matches, lw=2)
                plot_matches(world_pts,
                             ref_pts)
                plt.show()
                plt.clf()
...
def processing_rim(gaze_data, 
    ...
    config = {
        'processing': {
            'downsampling_factor': kwargs.get("downsampling_factor", 5),
            'max_time': kwargs.get('max_time', 5),
            'min_line_matches': kwargs.get('min_line_matches', 0),


            'min_line_matches': kwargs.get('min_line_matches', 0),
            'min_point_matches': kwargs.get('min_point_matches', 100),

            'camera': {
                'width': kwargs.get('original width', 1600),
            ...
            'files': {
                'outputDir': kwargs.get('outputDir', 'mappedGazeOutput'),
...
if __name__ == '__main__':
    processing_rim(gaze_data = 'input/_gaze.csv',
